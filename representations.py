...
class CompositeGrid(object):
    ...
    def update(self):
        ...
        assert self.frames[1]<=self.nframe
        print ("Number of frames found: %d"%self.nframe)
        print ("Frames %d --> %d  will be used"%(self.frames[0],self.frames[1]))
        print ("Number of elements in each frame: %d" %self.max_size)
    def getRep(self,*args,**kwargs):
        kw = {}
        kw.update(kwargs)
        self.rep = kw.get('rep','fbd')
        self.X = []
        self.y = []
        frame_counter = 0
        ...
        with open (self.xfile) as fx:
            while True:
                ...
                if not buf: break
                natom = int(buf)
                target = float(fx.readline().split()[-1])
                data = []
                for i in range(natom):
                    data.append([float(elem) for elem in fx.readline().split()])
                data = np.array(data)
                data = data[data[:,0].argsort()]
                if self.frames[0]<=frame_counter < self.frames[1]:
                    self.y.append(target)
                    allX = []
                    if self.rep in ['fbd','all']:
                        #self.X.append(data[:,4])
                        #print("1")
                        allX.append(data[:,4])
                    if self.rep in ['fnbd','all']:
                        #self.X.append(np.array([elem+1 for elem in data[:,4]]))
                        allX.append(np.array([elem+1 for elem in data[:,4]]))
                        #print("2")
                    if self.rep in ['son','all']: #sum of neighbours
                        #print("3")
                        grid = data[:,4]
                        assert int(np.sqrt(len(grid))+0.5)**2 == len(grid) 
                        ...
                        for i in range(n):
                            for j in range(n):
                                ...
                                try:
                                    ...
                                    bot = 0.0
                                new_grid[i][j] = grid[i][j] + left + right + top + bot
                        #self.X.append(new_grid.flatten())
                        allX.append(new_grid.flatten())
                    if self.rep in ['cust']:
                        grid = data[:,4]
                        n = len(grid)
                        x = data[:,1]
                        ...
                        threepoint_mat = np.zeros ((n,n,n))
                        for i in range(n):
                            dist_mat[i][i] = (grid[i]+1)**2
                            a = np.array([x[i],y[i]])
                            for j in range(i):
                                b = np.array([x[j],y[j]])
                                dist_mat[i][j] = dist_mat[j][i] =\
                                    (grid[i]+1)*(grid[j]+1)/np.linalg.norm(a-b)
                        #allX.append(list(collections.Counter(grid).values()))
                        allX.append(np.append(np.append(np.tensordot(x,grid,axes=0).flatten(),\
                                              np.tensordot(y,grid,axes=0).flatten()),\
                                                  dist_mat.flatten()))
                    if self.rep in ['dist', 'all']:
                        grid = data[:,4]
                        n = len(grid)
                        #grid = np.array(grid).reshape(n,n)
                        ...
                        for i in range(n):
                            ...
                            for j in range(i):
                                ...
                                dist_mat[i][j] = dist_mat[j][i] =\
                                    (grid[i]+1)*(grid[j]+1)/np.linalg.norm(a-b)
                                #for k in range(j):
                                #    area = PolyArea([x[i],x[j],x[k]],[y[i],y[j],y[k]])
                                #    threepoint_mat[i][j][k] = \
                                #            (grid[i]+1)*(grid[j]+1)*(grid[k]+1)*area
                                
                        #allX.append(-1.*np.sort(-1*dist_mat.flatten())) 
                        allX.append(dist_mat.flatten()) 
                        #allX.append(np.append(dist_mat.flatten(),threepoint_mat.flatten())) 
                        #allX.append(np.linalg.eig(dist_mat)[1].flatten())
                    if self.rep in ['spring']: #parallel series model spring
                        grid = data[:,4]
                        assert int(np.sqrt(len(grid))+0.5)**2 == len(grid) 
                        n = int(np.sqrt(len(grid)))
                        grid = np.array(grid).reshape(n,n)
                        v1 = np.mean(grid,axis=0)
                        v2 = np.mean(grid,axis=1)
                        
                        allX.append(np.vstack((v1,v2)).flatten())
                    ...
                    pass
                frame_counter += 1
                #print ("Entry:\n",np.array(allX).flatten())
                #allX.append([elem**2 for elem in allX])
                self.X.append(np.array(allX).flatten())   
                bar.update()
        self.X = np.array(self.X)
        self.y = np.array(self.y)
...
class StructureList(object):
    ...
    def getRep(self,*args,**kwargs):
        ...
        self.nolabel = kw.get('nolabel',False)
        self.stats = None
        self.X=[]
        self.y=[]
        self.data = None
        ...
        print("Generating representations")
        if self.nolabel:
            self.yfile = 'default_label.txt'
            with open (self.yfile, "w") as fp:
